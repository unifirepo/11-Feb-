export const homeContent = {
  hero: {
    // Hero carousel (rotates every 5s)
    // Uses the provided hero-1a/b/c/d images.
    slides: [
      {
        title: 'The greatest asset of any organisation is its people.',
        caption: 'Every building exists to serve, protect, and optimise their wellbeing.',
        image: '/unifi-assets/hero-1a.webp',
        cta: { text: 'Explore Platform', href: '/platform/overview' },
      },
      {
        title: 'Smarter Buildings.',
        caption:
          'Unifi.id helps you understand how people use your spaces - giving you the insight to protect lives, reduce risk, and improve operational performance.',
        image: '/unifi-assets/hero-1b.webp',
        cta: { text: 'View Solutions', href: '/solutions/hub' },
      },
      {
        title: 'Safer People.',
        caption:
          'Real-time intelligence that protects people, strengthens compliance, and optimises your entire estate.',
        image: '/unifi-assets/hero-1c.webp',
        cta: { text: 'Discover FireGuard', href: '/solutions/fireguard' },
      },
      {
        title: 'Greener Futures.',
        caption:
          'Reduce waste and prioritise upgrades with evidence-led building intelligence that connects safety, occupancy, and energy.',
        image: '/unifi-assets/hero-1d.webp',
        cta: { text: 'Explore Energy', href: '/energy/hub' },
      },
    ],
  },
  challenge: {
    title: 'The Challenge We Solve',
    intro: 'Modern estates are more complex than ever - but most are still managed with outdated tools and disconnected systems.',
    cards: [
      {
        title: 'Budgets',
        image: '/unifi-assets/corporate/photo-1556761175-4b46a572b786.webp',
        description: 'Budgets are drained because inefficiency is built into infrastructure that hasn\'t kept pace with demand.',
      },
      {
        title: 'Safety',
        image: '/unifi-assets/corporate/photo-1521737711867-e3b97375f902.webp',
        description: 'Safety is compromised because outdated fire drills, roll-calls, and door logs can\'t confirm who is inside, where they are, or whether systems are working as they should. Evacuation oversight, behavioural anomaly detection, and real-time fire system monitoring are often missing - leaving people and compliance exposed.',
      },
      {
        title: 'Energy',
        image: '/unifi-assets/canary/photo-1693079678487-2a90cd938d8f.webp',
        description: 'Energy is wasted because lighting and HVAC run when no one is there, inflating costs and carbon footprints. With carbon taxes and ESG frameworks tightening, estates are under growing pressure to decarbonise - yet few have the capital to act.',
      },
      {
        title: 'Compliance',
        image: '/unifi-assets/canary/photo-1694980876919-dbec5edba9a2.webp',
        description: 'Compliance is fragile because reporting is manual, time-consuming, and often incomplete.',
      },
    ],
    closing: 'Each system works in isolation - security, lighting, fire safety, energy - delivering fragments of the truth. What\'s missing is a single intelligence layer that brings it all together. That\'s what Unifi.id delivers: the ability for your buildings to see, understand, and act - in real time, with people at the centre.',
  },
  cortex: {
    title: 'Introducing Cortex™ - the Unifi.id real-time intelligence platform',
    description: 'Cortex™ is the intelligence layer that connects people, safety, energy, and operations into one living system.',
    intro: 'Instead of silos and blind spots, leadership teams gain real-time visibility across their entire estate:',
    cta: {
      text: 'Explore the platform',
      href: '/platform/overview',
    },
    pillars: [
      {
        title: 'Total Visibility',
        image: '/unifi-assets/corporate/photo-1600880292203-757bb62b4baf.webp',
        description: 'Know who is inside, where they are, and how spaces are really used.',
      },
      {
        title: 'Safer Estates',
        image: '/unifi-assets/canary/photo-1694902304056-b4ba1412d7b7.webp',
        description: 'See that safety systems are working, evacuations are managed, and compliance is assured.',
      },
      {
        title: 'Smarter Operations',
        image: '/unifi-assets/corporate/photo-1504384308090-c894fdcc538d.webp',
        description: 'Eliminate wasted energy, optimise utilisation, and meet ESG targets with proof.',
      },
    ],
    note: 'Cortex™ doesn\'t just collect data - it sets the stage for truly people-first building management.',
  },
  peopleFirst: {
    title: 'PEOPLE-FIRST, INTELLIGENCE-LED BUILDING MANAGEMENT',
    quote: 'Cortex™ is designed around one principle: buildings should work for people, not the other way around.',
    body: 'By uniting data from occupancy, fire safety, energy, and operations, Cortex™ translates complexity into clarity - giving leaders the confidence to make faster, smarter decisions. The result:',
    results: [
      {
        title: 'Smarter Buildings',
        image: '/unifi-assets/canary/photo-1692894115929-da265ede7d13.webp',
        description: 'Adapt spaces to demand, reducing waste and cost.',
      },
      {
        title: 'Safer People',
        image: '/unifi-assets/corporate/photo-1522202176988-66273c2fd55f.webp',
        description: 'Know who is inside, protect them with live safety oversight.',
      },
      {
        title: 'Greener Futures',
        image: '/unifi-assets/canary/photo-1694979296876-0889f68a8f27.webp',
        description: 'Cut energy use, prove carbon reductions, and stay compliant.',
      },
    ],
    cta: {
      text: 'Discover Cortex',
      href: '/platform/overview',
    },
  },
  solutions: {
    title: 'Choose Your Path to Smarter Buildings',
    intro: 'Whether you\'re focused on energy efficiency, fire safety, or comprehensive smart building intelligence, we have tailored solutions for your specific needs.',
    cards: [
      {
        title: 'Smart Building Platform',
        image: '/unifi-assets/canary/photo-1696229133286-c3f260384a3c.webp',
        description: 'Comprehensive occupancy intelligence, security, and analytics for complete building optimization.',
        bullets: [
          'Real-time occupancy tracking',
          'Advanced security solutions',
          'Predictive analytics',
          'Modular deployment options',
        ],
        cta: {
          text: 'View All Solutions →',
          href: '/solutions/hub',
        },
      },
      {
        title: 'FireGuard Platform',
        image: '/unifi-assets/corporate/photo-1525130413817-d45c1d127c42.webp',
        description: 'Complete fire safety intelligence with real-time monitoring, compliance, and insurance integration.',
        bullets: [
          'Real-time fire system monitoring',
          'Insurance risk reduction',
          'Compliance automation',
          'Independent or integrated deployment',
        ],
        cta: {
          text: 'Discover FireGuard →',
          href: '/solutions/fireguard',
        },
      },
      {
        title: 'Energy Solutions',
        image: '/unifi-assets/canary/photo-1696074010622-65271d2f423b.webp',
        description: 'Fully funded LED, heating, and solar upgrades with guaranteed savings and zero upfront costs.',
        bullets: [
          'Zero upfront investment',
          'Guaranteed energy savings',
          'Complete project management',
          'Multiple funding options',
        ],
        cta: {
          text: 'Explore Energy Solutions →',
          href: '/energy/hub',
        },
      },
    ],
  },
  sectors: {
    title: 'Solutions Built For Your World',
    intro: 'Every estate has unique challenges. Whether you manage schools, public estates, or corporate portfolios, Unifi.id delivers people-first intelligence that adapts to your needs.',
    bySector: {
      title: 'By Sector',
      items: [
        'Education – from state schools to independent groups, creating brighter, safer learning environments with no capital drain.',
        'Public Sector – councils, NHS, emergency services, and government estates seeking efficiency, compliance, and decarbonisation.',
        'Private Sector – from corporate offices to hospitality, transform operations with real-time intelligence that improves safety and efficiency.',
      ],
      cta: {
        text: 'Explore Sectors',
        href: '/sectors/hub',
      },
    },
    byRole: {
      title: 'By Role',
      items: [
        'Finance Leaders – protect budgets, unlock upgrades from savings, and reduce long-term risk exposure.',
        'Operational Leaders – gain total visibility across estates to improve resilience, streamline workflows, and optimise resources.',
        'Safety & Compliance Leads – safeguard people and automate compliance with real-time reporting and audit-ready data.',
        'Sustainability Leaders – deliver measurable carbon reductions and transparent ESG outcomes that withstand scrutiny.',
      ],
      cta: {
        text: 'Explore Roles',
        href: '/roles/hub',
      },
    },
  },
  cta: {
    title: 'Ready to make your building smarter, safer, and greener?',
    description: 'Unifi.id delivers people-first building intelligence that cuts costs, increases safety, strengthens compliance, and accelerates your journey to net zero.',
    buttons: [
      {
        text: 'Book a Demo',
        href: '/contact',
      },
      {
        text: 'Talk to Us About Your Estate',
        href: '/contact',
      },
    ],
  },
};
